function [no,xo] = rel_hist(varargin)
%HIST  Histogram.
...
if nargout > 0
  ...
  if ~isempty(cax)
     histPatch = bar(cax,x,nn/numel(y),[miny maxy],'hist');
     if strcmpi(get(ancestor(cax,'figure'),'Visible'),'off')
         return;
     end     
  else
     if ~isequal( size(nn), size(binwidth)); binwidth = binwidth'; end 
     histPatch =  bar(x,nn./(numel(y).*binwidth),[miny maxy],'hist');
  end
